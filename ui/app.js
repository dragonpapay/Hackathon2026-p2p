...
function desarDocument(topic, tipus = 'text') {
  ...
  if (!existeix) {
    const nouDoc = {
      ...
      lastEdit: Date.now(), // <-- AFEGIT: Guardem el moment exacte de creació
...
function configurarEventosSwarm() {
  swarm.on('connection', (peer) => {
    ...
    // LÒGICA DE SINCRONITZACIÓ INICIAL AMB TIMESTAMP
    const textActual = editor.innerHTML
    if (textActual && textActual !== '<h1>Comença a escriure aquí...</h1>') {
      const docs = obtenirDocumentsDesats();
      const doc = docs.find(d => d.topic === topicActual);
      const meuTimestamp = doc && doc.lastEdit ? doc.lastEdit : 0; // Agafem la data de l'últim canvi

      // Enviem el missatge com a 'sync_inicial' junt amb la nostra data
      peer.write(b4a.from(JSON.stringify({ tipus: 'sync_inicial', contingut: textActual, timestamp: meuTimestamp })))
    }
    ...
    peer.on('data', data => {
      try {
        ...
        if (missatge.tipus === 'sync_inicial') {
          const docs = obtenirDocumentsDesats();
          const doc = docs.find(d => d.topic === topicActual);
          const miTimestamp = doc && doc.lastEdit ? doc.lastEdit : 0;

          // NOMÉS actualitzem si el seu document és estrictament MÉS NOU que el nostre
          if (missatge.timestamp > miTimestamp) {
            isUpdating = true
            editor.innerHTML = missatge.contingut
            guardarContingutDocument(topicActual, missatge.contingut)
            setTimeout(() => isUpdating = false, 50)
          }
        } else if (missatge.tipus === 'text') {
          // Lògica normal mentre s'està teclejant en temps real
          isUpdating = true
          editor.innerHTML = missatge.contingut
          guardarContingutDocument(topicActual, missatge.contingut)
          setTimeout(() => isUpdating = false, 50)
...
function guardarContingutDocument(topic, html) {
  ...
  if (docIndex !== -1) {
    ...
    docs[docIndex].lastEdit = Date.now(); // <-- AFEGIT: Actualitzem el timestamp cada cop que guardem
    localStorage.setItem('pears_documents', JSON.stringify(docs));
  }
}/** @typedef {import('pear-interface')} */
import Hyperswarm from 'hyperswarm'
